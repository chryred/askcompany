...
def post_new(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(post)  # TODO: get_absolute_url활용
    else:
        form = PostForm()
...
def user_page(request, username):
    ...
    post_list_count = post_list.count()  # 실제 데이터베이스에 count 쿼리를 던짐 len(post_list)는 전체 조회 후 메모리에 넣으므로 비효율적임

    if request.user.is_authenticated:
        # User객체, AnonymousUser
